class MalaysiaCalendar {
  ...
  getYearFromURL() {
    ...
    if (yearParam) {
      console.log("Year from URL:", yearParam);
    ...
    if (yearMatch) {
      // console.log("Year from URL path:", yearMatch[1]);
  ...
  updateNavigation() {
    ...
    if (monthYearElement) {
      if (this.currentView === "yearly") {
        ...
        monthYearElement.textContent = `${this.monthNames[this.currentMonth]} ${
          this.currentYear
        }`;
      }
    }
  ...
  async generateCalendarHTML() {
    ...
    // console.log("Pre-loaded month dates:", monthDates);
    ...
    for (let week = 0; week < 6; week++) {
      for (let day = 0; day < 7; day++) {
        const dateStr = formatDateLocal(renderDate); //.toISOString().slice(0, 10);
        ...
        if (holidayInfo && isCurrentMonth) classes += " holiday";
        ...
        if (isCurrentMonth) {
          ...
          html += `<div class="day-number ${
            isToday ? "clickable-today" : ""
          }" ${
            isToday ? `onclick="showTodayDetails('${dateStr}')"` : ""
          }>${renderDate.getDate()}</div>`;
          ...
          if (islamicDate) {
            html += `<div class="islamic-date clickable-islamic" onclick="showIslamicDateDetails('${dateStr}', '${
              islamicDate.day
            } ${islamicDate.monthName} ${islamicDate.year}')">${
              islamicDate.day
            } ${islamicDate.monthName.substring(0, 3)}</div>`;
          }
    ...
    for (let week = 0; week < 6; week++) {
      for (let day = 0; day < 7; day++) {
        ...
        if (isCurrentMonth) {
          ...
          if (islamicDate) {
            html += `<div class="islamic-date-yearly clickable-islamic" onclick="showIslamicDateDetails('${dateStr}', '${
              islamicDate.day
            } ${islamicDate.monthName} ${islamicDate.year}')">${
              islamicDate.day
            } ${islamicDate.monthName.substring(0, 3)}</div>`;
  ...
  renderHolidayList() {
    ...
    if (this.currentView === "yearly") {
      ...
      holidays = Object.entries(this.holidays)
        .filter(([date]) => {
          ...
          return (
            holidayDate.getMonth() === this.currentMonth &&
    ...
    if (holidays.length === 0) {
      html += `<p class="no-holidays">No holidays in ${
        this.currentView === "yearly"
          ? this.currentYear
          : this.monthNames[this.currentMonth] + " " + this.currentYear
      }.</p>`;
